...
Color& stringtoColor(const std::optional<std::string>& color = std::nullopt)
{
	Color Select_Color = Color::Black;
	if (color == "Red") { Select_Color = Color::Red; }
...
System::String^ filesystem2String(std::filesystem::path file) {
	System::String^ result = msclr::interop::marshal_as<System::String^>(file.string());
	return result;
}
std::string String2stdstring(System::String^ input) {
...
void WinForm::MainForm::openfile()
{
	...
	file << "output file =" << path << endl;
	file << "exe_dir path =" << exe_dir << endl;
}
...
void WinForm::MainForm::mouseClick(System::Windows::Forms::MouseEventArgs^ e) {
	...
	if (e->Button == System::Windows::Forms::MouseButtons::Left)
	{
		for (int i = 0; i < myList->Size(); i++)
		{
			...
			TempLineType = String2stdstring(MyLine->LineType);
			
			if (TempLineType == "Line" || TempLineType == "HLine" || TempLineType == "VLine")
			{
				if (MyLine->SelectLineTest(e->Location) == true)
				{
					MyLine->Selected = true;
					DrawAction = DrawAction0;
					MyLine->LineColor = stringtoColor(Select_Color);
		...
		for (int i = 0; i < myList->Size(); i++)
		{
			...
			{
				Select_Line_Type = String2stdstring(MyLine->LineType);				
				break;
			}
		}
	...
	if (e->Button == System::Windows::Forms::MouseButtons::Left && IsMouseDrag == true)
	{
		...
		if (DrawAction=="Draw") {		
			if (LineType == "Line") {
				...
				DrawLine->LineColor = stringtoColor(Select_Color);
				DrawLine->LineType = "Line";
				test = String2stdstring(DrawLine->LineType);
		...
		else if (DrawAction == "CrossLine") {
			{
				...
				if (ShiftKey == true)
				{
					...
					DrawLine1->LineColor = stringtoColor(Select_Color);
...
void WinForm::MainForm::redColorMenuItem() { 
	Select_Color = "Red"; 
	int i = Select_Line_Index;
	WinForm::Line^ MyLine = myList[i];
	MyLine->LineColor = stringtoColor(Select_Color);
...
void WinForm::MainForm::blueColorMenuItem() { 
	Select_Color = "Blue"; 
	int i = Select_Line_Index;
	WinForm::Line^ MyLine = myList[i];
	MyLine->LineColor = stringtoColor(Select_Color);
...
void WinForm::MainForm::yellowColorMenuItem() { 
	Select_Color = "Yellow"; 
	int i = Select_Line_Index;
	WinForm::Line^ MyLine = myList[i];
	MyLine->LineColor = stringtoColor(Select_Color);
...
void WinForm::MainForm::greenColorMenuItem() { 
	Select_Color = "Green"; 
	int i = Select_Line_Index;
	WinForm::Line^ MyLine = myList[i];
	MyLine->LineColor = stringtoColor(Select_Color);
...
void WinForm::MainForm::fuchsiaColorMenuItem() { 	
	Select_Color = "Fuchsia"; 	
	int i = Select_Line_Index;		
	WinForm::Line^ MyLine = myList[i];
	MyLine->LineColor = stringtoColor(Select_Color);
	MyLine->Selected = false;
...
void WinForm::MainForm::copyEvent() {
	...
	if (j >= 0)
	{
		...
		Select_Line_Index = j;
		Select_Line_Type = String2stdstring(Line->LineType);
	}
}
...
void WinForm::MainForm::deleteEvent() {
	int i = Select_Line_Index;	
	myList->RemoveAt(i);
	pictureBox1->Invalidate();
}
...
void WinForm::MainForm::savePicture() {
	
	Bitmap^ bmp = gcnew Bitmap(pictureBox1->Width, pictureBox1->Height);
	pictureBox1->DrawToBitmap(bmp, pictureBox1->ClientRectangle);
	std::filesystem::path pictureFile = exe_dir / "ScreenShot.png";	
	System::String^ pngFile = filesystem2String(pictureFile);	
