...
namespace WinForm {
    ...
    public:
        ...
        /*void changeColorAt(int index, Color color)
        {
            std::ofstream file("output.txt");
            file << "Color changed for line at index = " << index << " to Color= " << color.ToArgb() << std::endl;
            file << "Lines->Count = " << lines->Count << std::endl;
            if (index >= 0 && index < lines->Count) {
                lines[index]->LineColor = color;
                file << "Color changed for line at index " << index << " to " << color.ToArgb() << std::endl;
                file << "lines[index]->LineColor " << lines[index]->LineColor.ToArgb() << std::endl;
            }
            else
            {
                file << "Index out of bounds: " << index << std::endl;
            }
            file.close();
        }*/
